...
| Known detection gaps?                        | Yes, quantified                                          | Encoded and obfuscated representations are decoded before matching (7/7 and 7/10 on the adversarial corpus). Spelled-out digits and IPv6 are not detected. Person names and addresses need the off-by-default local NER detector, measured at 100% on the shipped cases |
