...
| Is there SSO, OIDC, or MFA?            | OIDC                                                     | Bearer tokens from a configured OpenID Connect issuer are verified against its published keys and mapped to a workload and role; MFA is the issuer's concern. Static keys remain for services without an issuer            |
| Is there role-based access control?    | Two roles                                                | Caller keys submit requests and read their own workload's receipts; auditor keys, issued separately, read every workload's receipts and the metrics and cannot submit. Policy differentiates by workload and by data class |
| Is access to the gateway rate limited? | Per workload                                             | A token bucket per workload key (`policy.rateLimit`, globally or per workload) answers 429 with Retry-After; it is per replica, so keep a rate-limiting ingress in front for untrusted-adjacent workloads                  |
...
| Where do keys live?                   | Environment, or a remote signer                                  | Client and provider keys are environment variables from the secret manager. The receipt signing key can stay out of the gateway entirely with `receiptSigner.kind = remote` (HSM, Vault Transit, or a KMS adapter); the gateway then holds only the public key and verifies every signature it receives |
...
| Known detection gaps?                        | Yes, quantified                                          | Encoded and obfuscated representations are decoded before matching (7/7 and 7/10 on the adversarial corpus). Spelled-out digits are not detected. Person names, addresses, and organisations need the off-by-default local NER detector, measured at 100% on the shipped cases |
...
| Question                                       | Answer                                                       | Evidence and limits                                                                                                                                                                                                                                                    |
| ---------------------------------------------- | ------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Is there a compatibility and support policy?   | Yes                                                          | `docs/COMPATIBILITY.md` freezes the API, receipt schema, configuration schema and integration contracts; receipts from any release verify forever; breaking changes only at a minor version, announced a release ahead; previous minor gets security fixes for 90 days |
...
| High availability                              | Per-replica chains                                           | Each replica owns a receipt chain (`EGRYSA_RECEIPT_CHAIN_SUFFIX`, a StatefulSet manifest is provided); evidence export anchors every chain off the pod, and receipt lookup with several replicas goes through the export sink                                          |
